...
public:
               ...
               BacktestEngine& engine) override
    {
        ...
        while ((int)closes_.size() > maxNeeded) closes_.pop_front();
        if ((int)closes_.size() < maxNeeded) return;
...
private:
    double computeRSI() const {
        if ((int)closes_.size() < period_ + 1) return 50.0;
    ...
    double computeTrendSMA() const {
        if ((int)closes_.size() < trendWindow_) return closes_.back();
        double sum = 0.0;
        int n = (int)closes_.size();
        for (int i = n - trendWindow_; i < n; ++i) sum += closes_[i];

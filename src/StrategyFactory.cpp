                       ...
                       const std::string &symbol)
{
  ...
  {
    std::size_t fastWindow = params.value("fast_window", static_cast<std::size_t>(10));
    std::size_t slowWindow = params.value("slow_window", static_cast<std::size_t>(30));
    return makeSmaCrossoverStrategy(symbol, fastWindow, slowWindow);
  }
  else if(name == "rsi_reversion")
  {
    std::size_t period = params.value("period", static_cast<std::size_t>(14));
  ...
  {
    std::size_t lookback = params.value("lookback_window", static_cast<std::size_t>(20));
  ...
  else if(name == "trend_rsi")
  {
    ...
    std::size_t trendWindow = params.value("trend_window", static_cast<std::size_t>(50));
